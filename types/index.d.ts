...
export interface Descriptor<E, V> {
  ...
  connect?(host: E & HTMLElement & { __property_key__: V }, key: "__property_key__", invalidate: (options?: { force?: boolean }) => void): Function | void;
...
export type Component<E> = ComponentBase & {
  [property in Extract<keyof Omit<E, keyof HTMLElement>, string>]: property extends "render" | "content"
...
export namespace define {
  ...
  function from(components: { [path: string]: Component<any> }, options?: { prefix?: string; root?: string | string[] }): void;
}
...
export function parent<E, V>(componentOrFn: Component<V> | ((component: Component<E>) => boolean)): Descriptor<E, V>;
...
export type Model<M extends ModelInstance> = NonArrayObject & {
  [property in keyof Omit<M, "id">]-?: NonNullable<M[property]> extends Array<any>
    ?
        | NestedArrayModel<NonNullable<M[property]>>
        | (NonConstructor & ((model: M) => undefined extends M[property] ? undefined | NestedArrayModel<M[property]> : NestedArrayModel<M[property]>))
        ...
        : NonNullable<M[property]> extends boolean | Boolean
          ...
          : NonNullable<M[property]> extends ModelInstance
            ?
                ...
                | (NonConstructor &
                    ((model: M) => undefined extends M[property] ? undefined | Model<NonNullable<M[property]>> : Model<NonNullable<M[property]>>))
            : NonNullable<M[property]> extends NonArrayObject
              ? NonNullable<M[property]> | (NonConstructor & ((model: M) => M[property]))
              : never;
...
export type Storage<M extends ModelInstance> = {
  ...
  list?: (id: ModelIdentifier) => Array<StorageResult<M>> | Promise<Array<StorageResult<M>>>;
...
export namespace store {
  ...
  function value<M extends ModelInstance>(defaultValue: string, validate?: ValidateFunction<M, string> | RegExp, errorMessage?: string): string;
  function value<M extends ModelInstance>(defaultValue: number, validate?: ValidateFunction<M, number> | RegExp, errorMessage?: string): number;
  function value<M extends ModelInstance>(defaultValue: boolean, validate?: ValidateFunction<M, number> | RegExp, errorMessage?: string): boolean;
}
...
export function router<E>(
  views: ComponentBase | ComponentBase[] | (() => ComponentBase | ComponentBase[]),
  options?: {
    url?: string;
...
export namespace router {
  ...
  function url<E>(view: ComponentBase, params?: UrlParams<E> & UrlOptions): URL | "";
  ...
  function active(views: ComponentBase | ComponentBase[], options?: { stack?: boolean }): boolean;
...
export function localize(
  translate: (key: string, context: string) => string | ((num: number) => string),
  options?: {
    format?: "chrome.i18n";
  },
): void;
...
export namespace msg {
  function html<E>(parts: TemplateStringsArray, ...args: unknown[]): UpdateFunctionWithMethods<E>;

  function svg<E>(parts: TemplateStringsArray, ...args: unknown[]): UpdateFunctionWithMethods<E>;
}

/* Utils */

export function dispatch(host: EventTarget, eventType: string, options?: CustomEventInit): boolean;
...
export namespace html {
  ...
  function resolve<E>(promise: Promise<any>, placeholder?: UpdateFunction<E>, delay?: number): UpdateFunction<E>;
