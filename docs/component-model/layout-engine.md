...
It focuses only on "invisible" CSS rules, like display types, alignment, positioning, sizing, etc. The rest should be defined inside reusable UI components using the [styling](/component-model/templates.md#styling) methods supported by the templates.

## Usage

Add a `<template layout></template>` element wrapper with at least one empty `layout` attribute to the root template of the component. The `<template>` element represents the host element, which can also be styled:
...
While the template compiles, a group of attributes (`layout*="..."`) containing layout rules resolves to a single unique class selector added to the element. Generated selectors are shared between all component instances (regardless of whether the Shadow DOM is used or not). If the browser supports Constructable Stylesheets, the generated CSS is added to a single global shared stylesheet, which makes the styling as fast as possible.

### Other Styles

When using the layout engine inside UI components, you can still mix this approach with other styling techniques. Styles generated by the template helpers are added after the layout engine styles:
...
The `layout` attribute name can be extended with any valid CSS selector that does not contain a space character.

The value of the attribute does not support expressions, as the compilation process takes place before the function runs for the first time with arguments. However, the selector is added to the generated class selector, so the rules apply only when the conditions are met.
...
Media queries are supported via the part following the `@` character. For convenience, the engine defaults to the `min-width` media query feature. The engine supports the following values:
...
The attribute value contains a space-separated list of rules. The rules are applied in the order they appear in the attribute. Rules come with useful predefined arguments, but depending on the rule, they can take a list of arguments separated by the `:` character (without whitespace):
...
Add the `--` prefix to use a CSS variable as an argument. The engine passes it as is, without processing, so the variable can represent a dynamic value that can be updated at runtime, or scoped to some subtree.
...
CSS Grid layout is very flexible, but to make it easier to use, the engine maps the `[columns]` and `[rows]` arguments with the following rules:

* A number without a type suffix maps to `repeat([number], minmax(0, 1fr))`
* Otherwise, the value follows the [dimension](#dimensions) type
* Multiple rows/columns should be split with the `|` character, e.g.: `grid:1|min`
* Complex rules should not use whitespace, e.g.: `grid:repeat(2,minmax(50%,1fr))`
...
You can extend the ruleset by using the special empty rule `::`. It generates a CSS property with a value set to a CSS variable based on its name. In other words, it allows you to use predefined design tokens that map to any CSS properties.
