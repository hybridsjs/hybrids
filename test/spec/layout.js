  ...
  it("supports flexbox rules", () => {
    html`
      <template layout="row gap">
        ...
        <div><div layout="row:wrap inline"></div></div>
      </template>
    `(host);

    expect(window.getComputedStyle(host).gap).toBe("8px");

    expect(window.getComputedStyle(host.children[0]).flexGrow).toBe("1");
    expect(window.getComputedStyle(host.children[0]).flexShrink).toBe("2");

    expect(window.getComputedStyle(host.children[1]).flexShrink).toBe("1");
    expect(window.getComputedStyle(host.children[1]).flexGrow).toBe("2");
    expect(window.getComputedStyle(host.children[1]).order).toBe("2");
    expect(window.getComputedStyle(host.children[1]).flexBasis).toBe("auto");

    expect(window.getComputedStyle(host.children[2]).flexBasis).toBe("200px");

    expect(window.getComputedStyle(host.children[3].children[0]).flexWrap).toBe(
      "wrap",
    );
    expect(window.getComputedStyle(host.children[3].children[0]).display).toBe(
      "inline-flex",
    );
  });
